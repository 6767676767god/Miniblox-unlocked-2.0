new Promise(resolve => {
  ...
  if (script) {
    fetch(script.src).then(r => r.text()).then(code => {
      ...
      code = code.replaceAll('window.location.hostname.includes("miniblox.org")', '0');

      code = code.replace('=Game.isCrazyGames','=0');
      ...
      code = code.replace('index_browserExports.useFlag("update-cheats")', true);
  ...
  const openedAgo = m.server.timeAllocated
    ? "Opened " + msToTime(dayjs().diff(dayjs(m.server.timeAllocated))) + " ago"
    : null;
